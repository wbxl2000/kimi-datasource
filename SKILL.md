...
**不支持的能力**：通用 Web 搜索，以及 `xhcj` / `caixin` 覆盖之外的实时新闻。财经公告、快讯和政策新闻可以查（`xhcj` / `caixin`），但部分快讯接口的数据更新可能滞后、并非当日；回答新闻类问题时必须核对并标注数据的 PublishTime，不得把旧数据当作最新消息陈述。超出财经范围的新闻需求，告诉用户当前数据源不覆盖。
